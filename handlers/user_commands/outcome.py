...
@router.callback_query(F.data == CommandsCallbacks.write_outcome)
...
async def repeat_outcome(chat_id: int, state: FSMContext):
...
async def save_outcome(message: Message, state: FSMContext, session: AsyncSession):
    ...
    if message.text.isdigit():
        ...
        if amount < 0 or balance - amount < 0:
            await repeat_outcome(message.chat.id, state)
        else:
            balance -= amount
